...
4. Plots the results comparing the two implementations
"""
...
def process_tar_spdl(tar_data: bytes) -> int:
    ...
    """
    count = 0
    for _name, _content in spdl.io.iter_tarfile(tar_data):
        count += 1
    return count
...
def process_tar_builtin(tar_data: bytes) -> int:
    ...
    """
    count = 0
    for _name, _content in iter_tarfile_builtin(tar_data):
        count += 1
    return count
...
def process_tar_spdl_filelike(tar_data: bytes) -> int:
    ...
    for _name, _content in spdl.io.iter_tarfile(file_like):  # pyre-ignore[6]
        count += 1
    return count
...
def run_benchmark(
    ...
    """
    configs = [
        ("SPDL iter_tarfile (bytes)", process_tar_spdl),
        ("SPDL iter_tarfile (file-like)", process_tar_spdl_filelike),
        ("Python tarfile", process_tar_builtin),
    ]
